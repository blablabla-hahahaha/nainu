...
> 每个 note 是三件套：`foo.md` + `foo.zh.md` + `foo.i18n.yaml`。
...
#   pnpm run verify-translation-pairing --write .agents/notes/<lifecycle>/<class>/<file>.md
<file>.md: <git-hash-object 结果>
<file>.zh.md: <git-hash-object 结果>
```
...
- implemented → `archived/`：移动完整三件套，不改正文，插入 `Archived: YYYY-MM-DD` 行，重录 sidecar，修/删入站链接，`verify-archived-agent-notes --write` 追加哈希，之后永久冻结。
